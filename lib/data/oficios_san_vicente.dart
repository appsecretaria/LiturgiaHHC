import '../models/oficio.dart';
import '../models/tipo_oficio.dart';

const Oficio primerasVisperasSanVicente = Oficio(
  fecha: '26 de septiembre',
  tipo: TipoOficio.primerasVisperas,

  himnoLiturgico:
      '¿Qué nuevo triunfo cantan, hoy, los cielos?\n'
      '¿qué nuevo aplauso los santos tributan?\n'
      '¡La luz del clero, el Padre de los pobres\n'
      'brilla en la Gloria!\n\n'
      'Obras ingentes loan tus proezas\n'
      'y el Amor ciñe de laurel tu frente:\n'
      'cuanto le diste al pobre con largueza\n'
      'te vuelve el Cielo.\n\n'
      'Los sacerdotes, siendo tú su guía,\n'
      'llevan al pobre la verdad de Cristo:\n'
      'la madre Iglesia vive y canoniza\n'
      'tu Magisterio.\n\n'
      'Pero te honran de manera insigne\n'
      'vírgenes castas que, a la vez, son madres:\n'
      'los pobres gozan, bajo tu mirada,\n'
      'de su ternura.\n\n'
      'Como aliviaste el dolor del mísero\n'
      'oye hoy, benigno, el clamor del pueblo:\n'
      'todos los pobres, juntos te proclaman\n'
      'Padre y amigo.\n\n'
      'Demos, hoy, todos, gloria al Padre Eterno\n'
      'y al Hijo Ungido Salvador del hombre\n'
      'y al Amor mismo, llama de Dios vivo\n'
      'que arde en Vicente. ¡Amén!',

  antifona1:
      'Los pobres y necesitados buscan agua y no la encuentran; '
      'yo el Señor los oiré; yo el Dios de Israel no los abandonaré.',

  salmo1Titulo: 'Salmo 112',

  salmo1:
      'Alabad, siervos del Señor,\n'
      'alabad el nombre del Señor.\n'
      'Bendito sea el nombre del Señor,\n'
      'ahora y por siempre:\n'
      'de la salida del sol hasta su ocaso\n'
      'alabado sea el nombre del Señor.\n\n'
      'El Señor se eleva sobre todos los pueblos,\n'
      'su gloria sobre los cielos.\n'
      '¿Quién como el Señor Dios nuestro,\n'
      'que se eleva en su trono\n'
      'y se abaja para mirar\n'
      'al cielo y a la tierra?\n\n'
      'Levanta del polvo al desvalido,\n'
      'alza de la basura al pobre,\n'
      'para sentarlo con los príncipes,\n'
      'los príncipes de su pueblo;\n'
      'a la estéril le da un puesto en la casa,\n'
      'como madre feliz de hijos.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  antifona2: 'Os daré pastores según mi corazón y os apacentarán en ciencia y en doctrina.',

  salmo2Titulo: 'Salmo 145',

  salmo2:
      'Alaba, alma mía, al Señor:\n'
      'alabaré al Señor mientras viva,\n'
      'tañeré para mi Dios mientras exista.\n\n'
      'No confiéis en los príncipes,\n'
      'seres de polvo que no pueden salvar;\n'
      'exhalan el espíritu y vuelven al polvo,\n'
      'ese día perecen sus planes.\n\n'
      'Dichoso a quien auxilia el Dios de Jacob,\n'
      'el que espera en el Señor, su Dios,\n'
      'que hizo el cielo y la tierra,\n'
      'el mar y cuanto hay en él;\n\n'
      'que mantiene su fidelidad perpetuamente\n'
      'que hace justicia a los oprimidos,\n'
      'que da pan a los hambrientos.\n\n'
      'El Señor liberta a los cautivos,\n'
      'el Señor abre los ojos al ciego,\n'
      'el Señor endereza a los que ya se doblan,\n'
      'el Señor ama a los justos.\n\n'
      'El Señor guarda a los peregrinos,\n'
      'sustenta al huérfano y a la viuda\n'
      'y trastorna el camino de los malvados.\n\n'
      'El Señor reina eternamente,\n'
      'tu Dios, Sión, de edad en edad.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  antifona3:
      'Embriagaré el alma de mis sacerdotes con la abundancia '
      'y colmaré a mi pueblo de mis bienes.',

  salmo3Titulo: 'Cántico',
  salmo3Referencia: 'Ef 1,3-10',

  salmo3:
      'Bendito sea Dios,\n'
      'Padre de nuestro Señor Jesucristo,\n'
      'que nos ha bendecido en la persona de Cristo\n'
      'con toda clase de bienes espirituales y celestiales.\n\n'
      'Él nos eligió en la persona de Cristo,\n'
      'antes de crear el mundo,\n'
      'para que fuésemos santos\n'
      'e irreprochables ante él por el amor.\n\n'
      'Él nos ha destinado en la persona de Cristo,\n'
      'por pura iniciativa suya,\n'
      'a ser sus hijos,\n'
      'para que la gloria de su gracia,\n'
      'que tan generosamente nos ha concedido\n'
      'en su querido Hijo,\n'
      'redunde en alabanza suya.\n\n'
      'Por este Hijo, por su sangre,\n'
      'hemos recibido la redención,\n'
      'el perdón de los pecados.\n'
      'El tesoro de su gracia, sabiduría y prudencia\n'
      'ha sido un derroche para con nosotros,\n'
      'dándonos a conocer el Misterio de su Voluntad.\n\n'
      'Éste es el plan que había proyectado realizar por Cristo\n'
      'cuando llegase el momento culminante:\n'
      'recapitular en Cristo todas las cosas\n'
      'del cielo y de la tierra.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  lecturaBreveTitulo: 'Lectura breve',
  lecturaBreveReferencia: '1 Jn 4,19-21',

  lecturaBreve:
      'Nosotros amamos a Dios, porque Él nos amó primero. '
      'Si alguno dice: Amo a Dios y aborrece a su hermano, '
      'es un mentiroso, pues quien no ama a su hermano a quien ve, '
      'no puede amar a Dios a quien no ve. '
      'Y hemos recibido de Él este mandamiento: '
      'Quien ama a Dios, ame también a su hermano.',

  responsorio:
      'V/ Me gozaré y alegraré en tu misericordia.\n'
      'R/ Me gozaré y alegraré en tu misericordia.\n'
      'V/ Porque has mirado mi aflicción.\n'
      'R/ En tu misericordia.\n'
      'V/ Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'R/ Me gozaré y alegraré en tu misericordia.',

  antifonaCantico:
      'El Espíritu del Señor está sobre mí, '
      'me envió a evangelizar a los pobres.',

  introduccionPreces:
      'Al recordar con alegría a San Vicente de Paúl, '
      'imploremos la tierna bondad de Dios, para que despierte '
      'y renueve siempre en la Familia Vicenciana el espíritu '
      'de nuestro Padre, y digámosle con confianza:',

  respuestaPreces: 'Inflámanos en tu amor.',

  preces: [
    PreceOficio(
      inicio: 'Tú que enviaste a tu Hijo para que trajera la Buena Noticia a los pobres,',
      respuesta: 'que jamás deje la Iglesia de anunciar la Palabra Salvadora.',
    ),
    PreceOficio(
      inicio: 'Tú que llamaste a la Familia Vicenciana a participar en la misión de tu Hijo,',
      respuesta: 'concédele tener los mismos sentimientos y afectos llenándose de su espíritu.',
    ),
    PreceOficio(
      inicio: 'Que aprendamos realmente a amar a nuestro pueblo y a todo ser humano,',
      respuesta:
          'y a construir una convivencia humana en la paz y en el bienestar.',
    ),
    PreceOficio(
      inicio: 'Tú que cifraste en el amor la perfección de la ley, muévenos a compartir el gozo de los alegres y la pena de los tristes,',
      respuesta: 'para que nuestro amor revierta más en bien de todos.',
    ),
    PreceOficio(
      inicio: 'Tú que hiciste a todos según tu imagen,',
      respuesta:
          'haz que nuestra entrega contribuya a luchar contra las injusticias.',
    ),
    PreceOficio(
      inicio: 'Tú que suscitas continuamente el propósito santo de dar la vida para servirte a ti y a los hermanos,',
      respuesta: 'sosténnos con la fuerza del Espíritu que nos llama a ser en el mundo testigos del amor de Cristo.',
    ),
    PreceOficio(
      inicio: 'Que quienes se esforzaron por reconocerte y amarte en los más pobres,',
      respuesta:
          'lleguen a contemplarte, cara a cara, por siempre en tu Reino.',
    ),
  ],

  oracionConclusiva:
      'Señor Dios nuestro, que para la evangelización de los pobres '
      'y la formación del clero, infundiste en tu bienaventurado sacerdote '
      'Vicente las virtudes apostólicas, haz, te rogamos, que los que seguimos '
      'el ejemplo de su vida sintamos de continuo la urgencia de la caridad '
      'para perpetuar la misión de tu Hijo en el mundo. '
      'Por nuestro Señor Jesucristo, tu Hijo, que vive y reina contigo '
      'en la unidad del Espíritu Santo y es Dios, por los siglos de los siglos. Amén.',
); // I Vísperas

const Oficio laudesSanVicente = Oficio(
  fecha: '27 de septiembre',
  tipo: TipoOficio.laudes,

  himnoLiturgico:
      'Venid, pobres, que fuisteis los amores\n'
      'de Vicente, cantemos su bondad.\n'
      'Él las llagas curó de los dolores\n'
      'con besos de ternura y caridad.\n'
      'Cantemos al Apóstol de los campos,\n'
      'que es imagen de Cristo Redentor.\n'
      'Gloria al Santo que huyó la humana gloria;\n'
      'amor al Padre que vivió de amor.\n\n'
      'Oh, Padre de los tristes, oh, cáliz de consuelo.\n'
      'En ti está el Dios del cielo,\n'
      'que Dios es todo amor.\n'
      '¿Quién como tú amó al pobre,\n'
      'su dulce Padre y Guía?\n'
      'Reíste en su alegría,\n'
      'lloraste en su dolor.\n\n'
      'Ser en la tierra Cristo, tu vida fue y tu gloria;\n'
      'el lema de tu historia,\n'
      'pasar haciendo el bien.\n'
      'El campo de miserias regaron tus sudores;\n'
      'la dicha abrió sus flores,\n'
      'trocándole en Edén.\n\n'
      'Aún vive en tus Apóstoles el alma de tu celo,\n'
      'tus Hijas en el suelo\n'
      'han del triste piedad.\n'
      'Tu vida será eterna, eterna cual tu gloria;\n'
      'lo eterno es la victoria\n'
      'de tu alta caridad.\n\n'
      'El pobre en su miseria, y el triste en su quebranto,\n'
      'y el huérfano en su llanto,\n'
      'quien sufre viene aquí.\n'
      'Y a ti elevan sus ojos llorando sus dolores,\n'
      'a ti, fuente de amores,\n'
      'que está el consuelo en ti.',

  antifona1: 'Que tus sacerdotes, Señor, se vistan de justicia y se alegren tus santos.',

  salmo1Titulo: 'Salmo 62',
  salmo1Subtitulo: 'El alma sedienta de Dios',

  salmo1:
      '¡Oh Dios!, tú eres mi Dios, por ti madrugo,\n'
      'mi alma está sedienta de ti;\n'
      'mi carne tiene ansia de ti,\n'
      'como tierra reseca, agostada, sin agua.\n\n'
      '¡Cómo te contemplaba en el santuario\n'
      'viendo tu fuerza y tu gloria!\n'
      'Tu gracia vale más que la vida,\n'
      'te alabarán mis labios.\n\n'
      'Toda mi vida te bendeciré\n'
      'y alzaré las manos invocándote.\n'
      'Me saciaré de manjares exquisitos,\n'
      'y mis labios te alabarán jubilosos.\n\n'
      'En el lecho me acuerdo de ti\n'
      'y velando medito en ti,\n'
      'porque fuiste mi auxilio,\n'
      'y a la sombra de tus alas canto con júbilo;\n'
      'mi alma está unida a ti,\n'
      'y tu diestra me sostiene.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  antifona2: 'Sacerdotes del Señor, bendecid al Señor; siervos del Señor, entonad un himno a Dios.',

  salmo2Titulo: 'Cántico',
  salmo2Referencia: 'Dn 3,57-88.56',
  salmo2Subtitulo: 'Toda la creación alabe al Señor',

  salmo2:
      'Criaturas todas del Señor, bendecid al Señor,\n'
      'ensalzadlo con himnos por los siglos.\n'
      'Ángeles del Señor, bendecid al Señor;\n'
      'cielos, bendecid al Señor.\n\n'
      'Aguas del espacio, bendecid al Señor;\n'
      'ejércitos del Señor, bendecid al Señor.\n\n'
      'Sol y luna, bendecid al Señor;\n'
      'astros del cielo, bendecid al Señor.\n\n'
      'Lluvia y rocío, bendecid al Señor;\n'
      'vientos todos, bendecid al Señor.\n\n'
      'Fuego y calor, bendecid al Señor;\n'
      'fríos y heladas, bendecid al Señor.\n\n'
      'Rocíos y nevadas, bendecid al Señor;\n'
      'témpanos y hielos, bendecid al Señor.\n\n'
      'Escarchas y nieves, bendecid al Señor;\n'
      'noche y día, bendecid al Señor.\n\n'
      'Luz y tinieblas, bendecid al Señor;\n'
      'rayos y nubes, bendecid al Señor.\n\n'
      'Bendiga la tierra al Señor,\n'
      'ensálcelo con himnos por los siglos.\n\n'
      'Montes y cumbres, bendecid al Señor;\n'
      'cuanto germina en la tierra, bendiga al Señor.\n\n'
      'Manantiales, bendecid al Señor;\n'
      'mares y ríos, bendecid al Señor.\n\n'
      'Cetáceos y peces, bendecid al Señor;\n'
      'aves del cielo, bendecid al Señor.\n\n'
      'Fieras y ganados, bendecid al Señor,\n'
      'ensalzadlo con himnos por los siglos.\n\n'
      'Hijos de los hombres, bendecid al Señor;\n'
      'bendiga Israel al Señor.\n\n'
      'Sacerdotes del Señor, bendecid al Señor;\n'
      'siervos del Señor, bendecid al Señor.\n\n'
      'Almas y espíritus justos, bendecid al Señor;\n'
      'santos y humildes de corazón, bendecid al Señor.\n\n'
      'Ananías, Azarías y Misael, bendecid al Señor,\n'
      'ensalzadlo con himnos por los siglos.\n\n'
      'Bendigamos al Padre, al Hijo y al Espíritu Santo,\n'
      'ensalcémoslo con himnos por los siglos.\n\n'
      'Bendito el Señor en la bóveda del cielo,\n'
      'alabado y glorioso y ensalzado por los siglos.\n',

  rubricaSalmo2: 'No se dice Gloria',

  antifona3: 'Le concedió en la celebración de lo santo dignidad, por eso su memoria será bendita.',

  salmo3Titulo: 'Salmo 149',
  salmo3Referencia: null,
  salmo3Subtitulo: 'Alegría de los santos',

  salmo3:
      'Cantad al Señor un cántico nuevo,\n'
      'resuene su alabanza en la asamblea de los fieles;\n'
      'que se alegre Israel por su Creador,\n'
      'los hijos de Sión por su Rey.\n\n'
      'Alabad su nombre con danzas,\n'
      'cantadle con tambores y cítaras;\n'
      'porque el Señor ama a su pueblo\n'
      'y adorna con la victoria a los humildes.\n\n'
      'Que los fieles festejen su gloria\n'
      'y canten jubilosos en filas:\n'
      'con vítores a Dios en la boca\n'
      'y espadas de dos filos en las manos:\n\n'
      'para tomar venganza de los pueblos\n'
      'y aplicar el castigo a las naciones,\n'
      'sujetando a los reyes con argollas,\n'
      'a los nobles con esposas de hierro.\n\n'
      'Ejecutar la sentencia dictada\n'
      'es un honor para todos sus fieles.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  lecturaBreveTitulo: 'Lectura breve',
  lecturaBreveReferencia: 'Col 3,14-15',

  lecturaBreve:
      'Y por encima de todo esto, el amor, que es el ceñidor de la unidad consumada. '
      'Que la paz de Cristo actúe de árbitro en vuestro corazón; '
      'a ella habéis sido convocados, en un solo cuerpo. '
      'Y sed agradecidos.',

  responsorio:
      'V/ Sacerdotes del Señor. Bendecid al Señor.\n'
      'R/ Sacerdotes del Señor. Bendecid al Señor.\n'
      'V/ Santos y humildes de corazón alabad a Dios.\n'
      'R/ Bendecid al Señor.\n'
      'V/ Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'R/ Sacerdotes del Señor. Bendecid al Señor.',

  antifonaCantico: 'San Vicente fue consuelo de los que sufren, defensor de los huérfanos y protector de las viudas.',

  introduccionPreces: 'Bendigamos, llenos de alegría, al Padre que encomendó a Cristo un mensaje liberador para los pobres, y digámosle:',

  respuestaPreces: 'Señor, guía nuestras sendas en la claridad de tu rostro.',

  preces: [
    PreceOficio(
      inicio: 'Haz, Señor, que hoy vivamos en tu amor y en tu servicio,',
      respuesta: 'teniendo en Jesucristo nuestra referencia y modelo en el camino de la caridad.',
    ),
    PreceOficio(
      inicio: 'Concédenos construir en la fe, en el amor y en el servicio, nuestra comunidad,',
      respuesta: 'y dar así testimonio de la novedad del evangelio en medio de nuestros días.',
    ),
    PreceOficio(
      inicio:
          'Confirma nuestra consagración y multiplica en nosotros tu gracia,',
      respuesta: 'para que, revestidos por el Espíritu de Cristo, tendamos a la perfección de la caridad.',
    ),
    PreceOficio(
      inicio: 'Enséñanos a acoger a los necesitados y pobres que encontremos en nuestro camino,',
      respuesta: 'y a dar testimonio de tu amor paternal.',
    ),
    PreceOficio(
      inicio: 'Haznos voz autorizada de las necesidades y aspiraciones legítimas de los pobres,',
      respuesta:
          'especialmente de cuantos no tienen posibilidad de hacerse sentir.',
    ),
    PreceOficio(
      inicio: 'Ayúdanos a llenar el mundo de tu Espíritu,',
      respuesta: 'para que todos los hombres sientan tu presencia salvadora.',
    ),
  ],

  introduccionPadreNuestro: 'Ya que la fuerza para no caer en la tentación nos viene de Dios, acudamos al Padre, diciendo:',

  oracionConclusiva:
      'Señor Dios nuestro, que para la evangelización de los pobres y la formación del clero, '
      'infundiste en tu bienaventurado sacerdote Vicente las virtudes apostólicas, '
      'haz, te rogamos, que los que seguimos el ejemplo de su vida sintamos de continuo '
      'la urgencia de la caridad para perpetuar la misión de tu Hijo en el mundo. '
      'Él, que vive y reina contigo en la unidad del Espíritu Santo y es Dios, '
      'por los siglos de los siglos. Amén.',
); // Laudes

const Oficio segundasVisperasSanVicente = Oficio(
  fecha: '27 de septiembre',
  tipo: TipoOficio.segundasVisperas,

  himnoLiturgico:
      'Enséñanos a amar Vicente de Paúl\n'
      'al pobre nuestro hermano\n'
      'como lo amaste tú. (bis)\n\n'
      'No sabemos sufrir con los que sufren,\n'
      'rehusamos llorar con los que lloran,\n'
      'ignoramos la voz que nos suplica\n'
      'y la mano que hambrienta nos implora.\n\n'
      'Acallamos a veces entre rezos,\n'
      'el clamor de los pobres que nos gritan,\n'
      'con palabras de Cristo y su Evangelio\n'
      'que sólo es el amor lo que da vida.\n\n'
      'Vicente de Paúl que descubriste,\n'
      'a Cristo desvalido entre los pobres,\n'
      'que a la luz de tu vida descubramos,\n'
      'que ellos son nuestros Amos y Señores.',

  antifona1: 'Feliz al que has elegido, Señor: habitará en tus atrios.',

  salmo1Titulo: 'Salmo 14',
  salmo1Subtitulo: '¿Quién es justo ante el Señor?',

  salmo1:
      'Señor, ¿quién puede hospedarse en tu tienda\n'
      'y habitar en tu monte santo?\n\n'
      'El que procede honradamente\n'
      'y practica la justicia,\n'
      'el que tiene intenciones leales\n'
      'y no calumnia con su lengua,\n\n'
      'el que no hace mal a su prójimo\n'
      'ni difama al vecino,\n'
      'el que considera despreciable al impío\n'
      'y honra a los que temen al Señor,\n\n'
      'el que no retracta lo que juró\n'
      'aún en daño propio,\n'
      'el que no presta dinero a usura\n'
      'ni acepta soborno contra el inocente.\n\n'
      'El que así obra nunca fallará.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  antifona2: 'Repartió y dio con generosidad a los pobres, su justicia permanecerá por siempre.',

  salmo2Titulo: 'Salmo 111',
  salmo2Subtitulo: 'Felicidad del justo',

  salmo2:
      'Dichoso quien teme al Señor\n'
      'y ama de corazón sus mandatos.\n'
      'Su linaje será poderoso en la tierra,\n'
      'la descendencia del justo será bendita.\n\n'
      'En su casa habrá riquezas y abundancia,\n'
      'su caridad es constante, sin falta.\n'
      'En las tinieblas brilla como una luz\n'
      'el que es justo, clemente y compasivo.\n\n'
      'Dichoso el que se apiada y presta,\n'
      'y administra rectamente sus asuntos.\n'
      'El justo jamás vacilará,\n'
      'su recuerdo será perpetuo.\n\n'
      'No temerá las malas noticias,\n'
      'su corazón está firme en el Señor.\n'
      'Su corazón está seguro, sin temor,\n'
      'hasta que vea derrotados a sus enemigos.\n\n'
      'Reparte limosna a los pobres;\n'
      'su caridad es constante, sin falta,\n'
      'y alzará la frente con dignidad.\n\n'
      'El malvado, al verlo, se irritará,\n'
      'rechinará los dientes hasta consumirse.\n'
      'La ambición del malvado fracasará.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  antifona3: 'El Señor le concedió una gloria eterna y le hizo heredero de un nombre eterno.',

  salmo3Titulo: 'Cántico',
  salmo3Referencia: 'Ap 15,3-4',
  salmo3Subtitulo: 'Cántico de los vencedores',

  salmo3:
      'Grandes y maravillosas son tus obras,\n'
      'Señor, Dios omnipotente,\n'
      'justos y verdaderos tus caminos,\n'
      '¡oh Rey de los siglos!\n\n'
      '¿Quién no temerá, Señor,\n'
      'y glorificará tu nombre?\n'
      'Porque tú solo eres santo,\n'
      'porque vendrán todas las naciones\n'
      'y se postrarán en tu acatamiento,\n'
      'porque tus juicios se hicieron manifiestos.\n\n'
      'Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'Como era en el principio, ahora y siempre,\n'
      'por los siglos de los siglos. Amén.',

  lecturaBreveTitulo: 'Lectura breve',
  lecturaBreveReferencia: '1 Cor 13,4-7',

  lecturaBreve:
      'El amor es paciente, afable; no tiene envidia; '
      'no presume ni se engríe; no es mal educado ni egoísta; '
      'no se irrita, no lleva cuentas del mal; no se alegra de la injusticia, '
      'sino que goza con la verdad. Disculpa sin límites, cree sin límites, '
      'espera sin límites, aguanta sin límites.',

  responsorio:
      'V/ Dichoso el que cuida del pobre: en el día aciago lo pondrá a salvo el Señor.\n'
      'R/ Dichoso el que cuida del pobre: en el día aciago lo pondrá a salvo el Señor.\n'
      'V/ El Señor lo hará vivir feliz en la tierra.\n'
      'R/ En el día aciago lo pondrá a salvo el Señor.\n'
      'V/ Gloria al Padre, y al Hijo, y al Espíritu Santo.\n'
      'R/ Dichoso el que cuida del pobre: en el día aciago lo pondrá a salvo el Señor.',

  antifonaCantico: 'Saciaré de pan a los pobres de Sión, a sus sacerdotes los vestiré de salvación, y sus santos saltarán de alegría.',

  introduccionPreces:
      'Al recordar a San Vicente de Paúl con alegría, imploremos la bondad de Dios, '
      'para que despierte y renueve siempre en la Familia Vicentina el espíritu '
      'de nuestro Padre, y digámosle confiadamente:',

  respuestaPreces: 'Haznos auténticos testigos de tu amor.',

  preces: [
    PreceOficio(
      inicio: 'Padre, que has revelado el misterio de tu amor en el Verbo hecho carne, enviado a traer el anuncio de la salvación para todos los hombres,',
      respuesta: 'haz que encontremos en nuestro mundo, evangelizadores capaces de anunciar y hacer efectivo tu Evangelio.',
    ),
    PreceOficio(
      inicio: 'Concédenos un corazón sensible y abierto a las llamadas de las nuevas pobrezas de nuestro tiempo,',
      respuesta: 'para que, revestidos con el Espíritu de tu Hijo, vivamos el auténtico compromiso vicentino.',
    ),
    PreceOficio(
      inicio: 'Tú que nos has dado a los pobres como signo de tu presencia entre nosotros y los has hecho nuestros maestros y señores,',
      respuesta: 'haz que, evangelizándolos con amor y respeto, seamos nosotros, también, evangelizados.',
    ),
    PreceOficio(
      inicio: 'Haz que siguiendo a San Vicente de Paúl, sepamos descubrir en el amor la perfección de la ley,',
      respuesta: 'para que toda nuestra caridad revierta en bien de la paz y de la justicia.',
    ),
    PreceOficio(
      inicio: 'Tu Hijo llamó a los apóstoles y a los discípulos para que, unidos a tu Espíritu, fuesen evangelizadores eficaces de tu Reino en medio del mundo,',
      respuesta: 'haz que en nuestras Comunidades seamos un sólo corazón y una sola alma al servicio del Evangelio.',
    ),
    PreceOficio(
      inicio: 'Que quienes se esforzaron por reconocerte y amarte en los más pobres,',
      respuesta:
          'lleguen a contemplarte, cara a cara, por siempre en tu Reino.',
    ),
  ],

  introduccionPadreNuestro:
      'Porque nos llamamos y somos hijos de Dios, nos atrevemos a decir:',

  oracionConclusiva:
      'Señor Dios nuestro, que para la evangelización de los pobres y la formación del clero, '
      'infundiste en tu sacerdote San Vicente de Paúl las virtudes apostólicas, '
      'haz, te rogamos, que los que seguimos el ejemplo de su vida sintamos de continuo '
      'la urgencia de la caridad para perpetuar la misión de tu Hijo en el mundo. '
      'Por nuestro Señor Jesucristo, tu Hijo, que vive y reina contigo en la unidad '
      'del Espíritu Santo y es Dios, por los siglos de los siglos. Amén.',
);
